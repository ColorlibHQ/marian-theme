...
function marian_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'marian' ),
        'next_string'     => esc_html__( '&raquo;', 'marian' ),
        'before_output'   => '<div class="marian-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'marian_pagination_defaults', $defaults )
    );
