...
function marian_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'marian' ) . '</span></li>';
...
// Blog categories
function marian_featured_post_cat( $only_one = false ){
...
// social media
if ( ! function_exists( 'marian_social' ) ) {
	function marian_social( $args = array() ) {
		...
		$url = marian_opt( 'marian_social_url' );
...
// header cart count
function marian_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( MARIAN_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'marian' ); ?>">
...
function marian_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'marian_contact7_form_content', 10, 2 );