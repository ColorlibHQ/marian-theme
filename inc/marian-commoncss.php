...
// enqueue css
function marian_common_custom_css(){
    
    wp_enqueue_style( 'marian-common', get_template_directory_uri().'/assets/css/common.css' );

		$whiteColor     		  	= '#ffffff';
		$themeColor     		  	= marian_opt( 'marian_theme_color' ) != '#131313' ? marian_opt( 'marian_theme_color' ) : '';
		$themeSecColor     		  	= '';
		$bookBtnColor       		= marian_opt( 'marian_book_btn_color' ) != '#131313' ? marian_opt('marian_book_btn_color') : '';
		$bookBtnHoverBgColor     	= marian_opt( 'marian_book_btn_hvr_bg_color' ) != '#131313' ? marian_opt('marian_book_btn_hvr_bg_color') : $themeColor;
		$hoverColor     	  	  = marian_opt( 'marian_quote_btn_hvr_text_color');

		$headerTop_bg     		  = '';
		$navbarBgColor     		  = marian_opt( 'marian_navbar_bg_color' ) != '#082b49' ? marian_opt( 'marian_navbar_bg_color') : '';
		$headerTop_col     		  = marian_opt( 'marian_top_header_color' );

		$headerTopBg      		  = marian_opt( 'marian_top_header_bg_color');
		$topBtnBg          		  = marian_opt( 'marian_header_btn_bg_color') != '#ffffff' ? marian_opt( 'marian_header_btn_bg_color') : '';
		$topBtnHoverBg			  = marian_opt( 'marian_header_btn_hover_bg_color') != '#ffffff' ? marian_opt( 'marian_header_btn_hover_bg_color') : '';
		$headerBg          		  = marian_opt( 'marian_header_bg_color') != '#ffffff' ? marian_opt( 'marian_header_bg_color') : '';
		$menuColor          	  = marian_opt( 'marian_header_menu_color' ) != '#131313' ? marian_opt('marian_header_menu_color') : '';
		$menuHoverColor           = marian_opt( 'marian_header_menu_hover_color' ) != '#808080' ? marian_opt('marian_header_menu_hover_color') : $themeColor;
		$menuFixedHoverColor      = marian_opt( 'marian_header_menu_fixed_hover_color' );
		$dropMenuBgColor          = marian_opt( 'marian_header_drop_menu_bg_color' ) != '#ffffff' ? marian_opt('marian_header_drop_menu_bg_color') : $themeColor;
		$dropMenuColor            = marian_opt( 'marian_dropdown_menu_color' ) != '#131313' ? marian_opt('marian_dropdown_menu_color') : '';
		$dropMenuHovColor         = marian_opt( 'marian_dropdown_menu_hover_color' ) != '#808080' ? marian_opt('marian_dropdown_menu_hover_color') : $themeColor;

		$footerwbgColor     	  = marian_opt( 'marian_footer_widget_bg_color' );
		$footerwHeadColor   	  = marian_opt('marian_footer_widget_heading_color');
		$footerwTextColor   	  = marian_opt('marian_footer_widget_text_color');
		$footerwanchorcolor 	  = marian_opt('marian_footer_widget_anchor_color') != '#ffffff' ? marian_opt('marian_footer_widget_anchor_color') : '';
		$footerwanchorhovcolor    = marian_opt('marian_footer_widget_anchor_hover_color') != '#ffffff' ? marian_opt('marian_footer_widget_anchor_hover_color') : '';
		$footeraeroscrollcolor    = marian_opt('marian_footer_aeroplan_scroll_color') != '#dca73a' ? marian_opt('marian_footer_aeroplan_scroll_color') : '';
		
		$fofbg					  = marian_opt('marian_fof_bg_color');
		$foftonecolor			  = marian_opt('marian_fof_textone_color');
		$fofttwocolor			  = marian_opt('marian_fof_texttwo_color');

		$bannerBtnHeaderSpanColor = $themeColor != '#ff5e13' ? $themeColor : '';
		$footerAncDefColor 		  = marian_opt('marian_footer_widget_anchor_color') != '#9e9e9e' ? marian_opt('marian_footer_widget_anchor_color') : '';
		...
		$customcss ="			
			...
			.hero-banner, .banner_part .banner_text .btn_1, .abopt_number_counter .btn_1, .header-area .header-btn .btn1
			{
				background-color: {$topBtnBg};
			}
			.header-area .header-btn .btn1:hover
			{
				background-color: {$topBtnHoverBg};
			}
			.main_menu
			{
			...
			.main_menu.menu_fixed, .header-area.sticky
			{
				background: {$headerBg};
			}
			.header-area .main-header-area .main-menu ul li a, .header-area .main-header-area.sticky .main-menu ul li a, .header-area .social_wrap .login_text a, .header-area .social_wrap .number p a, .main-header .main-menu ul li a {
				color: {$menuColor};
			}
			...
			.main_menu .main-menu-item ul li .nav-link:not(.dropdown-item):hover, .header-area .main-header-area .main-menu ul li  a.dropdown-item:hover, .main_menu:not(.menu_fixed) ul.navbar-nav > li > a:hover, .header-area .social_wrap .login_text a:hover, .header-area .social_wrap .number p a:hover, .main-header .main-menu ul li:hover a
			{
			...
			.main_menu .main-menu-item ul.dropdown-menu li .nav-link, .dropdown .dropdown-menu .dropdown-item, li.menu-item-has-children .sub-menu li a, .header-area .main-header-area .main-menu ul li .submenu li a, .header-area .main-header-area.sticky .main-menu ul li .submenu li a, .main-header .main-menu ul ul.submenu > li > a
			{
			   color: {$dropMenuColor};
			}
			.header-area .main-header-area .main-menu ul li .submenu li a:hover, .header-area .main-header-area.sticky .main-menu ul li .submenu li a:hover, .main-header .main-menu ul ul.submenu > li > a:hover {
			...
			.footer_area .footer-bottom .social_links ul li a:hover, .footer-area .footer-social2 a:hover {
			...
			.footer-area .single-footer-widget .btn, .footer .footer_top .footer_widget .marian_mailchimp_newsletter_widget button, .footer .footer_top .footer_widget .marian_mailchimp_newsletter_widget button
			{
			...
			#scrollUp
			{
			   background: {$footeraeroscrollcolor};
			}
			.footer-area .single-footer-widget form button i
			{
			   color: {$footeraeroscrollcolor};
			}
    ...
    wp_add_inline_style( 'marian-common', $customcss );
    
}
add_action( 'wp_enqueue_scripts', 'marian_common_custom_css', 50 );