...
add_action( 'wp_enqueue_scripts', 'marian_sl_enqueue_scripts' );
function marian_sl_enqueue_scripts() {

	wp_enqueue_script( 'marian-post-likes', MARIAN_DIR_JS_URI . 'post-likes.js', array( 'jquery' ), '0.5', false );

	wp_localize_script( 'marian-post-likes', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'unlike'  => esc_html__( 'Unlike', 'marian' )
	) );
}
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_marian_process_simple_like', 'marian_process_simple_like' );
add_action( 'wp_ajax_marian_process_simple_like', 'marian_process_simple_like' );
function marian_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'marian' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'marian' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'marian' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'marian' );
	if ( is_numeric( $like_count ) && $like_count > 1 ) { 
		$number = sl_format_count( $like_count ) . esc_html__( ' Likes', 'marian' );
	} else {
		$number = sl_format_count( $like_count ) . esc_html__( ' Like', 'marian' );
	}
...
// User Profile List
add_action( 'show_user_profile', 'marian_show_user_likes' );
add_action( 'edit_user_profile', 'marian_show_user_likes' );
function marian_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'marian' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'marian' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
